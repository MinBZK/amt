...
async def test_remove_all_roles(db: DatabaseTestUtils):
    ...
    algorithms_repository.get_by_user = AsyncMock()
    algorithms_repository.get_by_user.return_value = [algorithm1, algorithm2]
    ...
    # then
    algorithms_repository.get_by_user.assert_called_once_with(UUID(str(user.id)))
...
async def test_get_by_id(db: DatabaseTestUtils):
    ...
    # Get the authorization to obtain its ID
    results = await authorization_repository.find_all(filters={"type": AuthorizationType.ORGANIZATION, "type_id": 1})
    assert len(results) == 1
    auth_id = results[0][1].id
