...
from amt.services.instruments_state import get_all_next_tasks , \
    get_instrument_result_from_system_card , get_task_timestamp_from_assessment_card , get_first_lifecycle_idx , \
    get_next_tasks_per_instrument , InstrumentStateService
...
def test_get_state_per_instrument(system_card: SystemCard):
    ...
    assert [{'urn': 'urn:nl:aivt:ir:aiia:1.0' , 'in_progress': 1 , 'name': 'AI Impact Assessment (AIIA)'} ,
            {'urn': 'urn:nl:aivt:ir:iama:1.0' , 'in_progress': 1 ,
             'name': 'Impact Assessment Mensenrechten en Algoritmes (IAMA)'} ,
            {'in_progress': 0 , 'name': 'URN not found in Task Registry.' ,
             'urn': 'urn:instrument:assessment'}] == res
...
def test_get_amount_completed_instruments_one_completed(system_card: SystemCard):
    instrument_state_service = InstrumentStateService(system_card)
    xo = instrument_state_service.get_state_per_instrument()
    instrument_state_service.instrument_states = [{
        "in_progress": 0
    } ,
        {
            "in_progress": 1
