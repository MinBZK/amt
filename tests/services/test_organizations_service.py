...
async def test_save() -> None:
    ...
    authorization_service.get_role.assert_called_once_with("Organization Maintainer")
    authorization_service.add_authorization.assert_called_once_with(
        default_user().id, default_organization().id, AuthorizationType.ORGANIZATION, 1
    )
