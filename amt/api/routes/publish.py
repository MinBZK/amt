...
async def publish_start(
    ...
    steps = [
        ...
        {"state": "incomplete", "line": "straight", "size": "md", "label": "Preview & controleren", "link": "./preview"},
...
# TODO: add new permission: PUBLISH
@permission({AuthorizationResource.ALGORITHM: [AuthorizationVerb.CREATE]})
@router.get("/{algorithm_id}/publish/preview")
...
# TODO: add new permission: PUBLISH
@permission({AuthorizationResource.ALGORITHM: [AuthorizationVerb.CREATE]})
@router.get("/{algorithm_id}/publish/confirm")
async def publish_confirm(
    ...
    if not publication:
        ...
        # Step 1: update existing algoritme
        create_result: PublicationResult = await update_algorithm(
            algorithm=algorithm,
            username=credentials.username,
            password=credentials.password,
            organisation_id=organisation_id,
            algorithm_id=publication.lars,
        )
