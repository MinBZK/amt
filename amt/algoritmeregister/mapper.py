...
from amt.schema.system_card import PublicationCategory
...
class AlgorithmMapper:
    ...
    def _map_publication_category(
        ...
        if system_card.ai_act_profile:
            pub_cat = getattr(system_card.ai_act_profile, "publication_category", None)
            if pub_cat:
                if isinstance(pub_cat, PublicationCategory):
                    pub_cat = pub_cat.value

                if isinstance(pub_cat, str):
                    pub_cat_lower = pub_cat.lower()
                    if "hoog-risico" in pub_cat_lower or "hoog risico" in pub_cat_lower:
                        return V10EnumPublicationCategory.HOOG_MINUS_RISICO_AI_MINUS_SYSTEEM
                    elif "impactvol" in pub_cat_lower:
                        return V10EnumPublicationCategory.IMPACTVOLLE_ALGORITMES
